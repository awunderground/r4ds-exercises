...
3. Use the appropriate lubridate function to parse the following dates:
...
6. What makes the distribution of `diamonds$carat` and `flights_dep_time` similar?
...
```{r flight_dep_time distribution}
flights_dt %>%
...
7. Confirm my hypothesis that the early departures of flights in minutes 20-30 and 50-60 are caused by scheduled flights that leave early. Hint: create a binary variable that tells whether or not the flight was delayed. 
...
```{r datetime vector transformation}
